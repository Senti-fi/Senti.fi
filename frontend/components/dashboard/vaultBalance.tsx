...
type RawVault = {
  ...
  vaultPubkey?: string;
  token?: string;
  amount?: number | string; // token units
  yieldRate?: number | null;
  lockedUntil?: string | null;
  isUnlocked?: boolean;
  rewardsAccrued?: number | string;
};
...
export default function VaultBalance() {
  ...
  const initialLoadedRef = useRef(false); // tracks whether first load happened

  // get live prices and aggregated map (from your existing hook)
  // we only need prices and bySymbol; the hook also loads token list/prices
  const { prices, bySymbol, solBalance } = useWalletBalancesRealtime("https://api.devnet.solana.com");

  const [vaults, setVaults] = useState<RawVault[] | null>(null);
  const [loading, setLoading] = useState<boolean>(false);
  const [fetchError, setFetchError] = useState<string | null>(null);

  // single fetch function that supports quiet refreshes
  async function loadVaults({ quiet = false }: { quiet?: boolean } = {}) {
    if (!user?.id) {
      if (!quiet) {
        setVaults([]);
      } else {
        // quiet: only update vaults if already set — do nothing
      }
      return;
    }
    ...
    try {
      ...
      const rawVaults = (resp.data?.vaults ?? []) as RawVault[];
      if (!mountedRef.current) return;
      setVaults(rawVaults);
      // mark initial load as completed (if it was the initial)
      initialLoadedRef.current = true;
    } catch (err: any) {
      console.error("VaultBalance: failed to load vaults", err);
      if (!mountedRef.current) return;
      const msg = err?.response?.data?.error ?? err?.message ?? "Failed to load vaults";
      if (!quiet) {
        setFetchError(msg);
        setVaults([]);
      } else {
        // quiet failure: keep previous vaults and optionally log error
        console.warn("Quiet vault refresh failed:", msg);
      }
    } finally {
      if (!mountedRef.current) return;
      if (!quiet) {
  ...
  // initial load (on mount or when user.id changes)
  useEffect(() => {
    mountedRef.current = true;
    initialLoadedRef.current = false;
    loadVaults({ quiet: false });

    return () => {
      mountedRef.current = false;
    };
    // only trigger when user.id changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id]);

  // Quiet refresh whenever solBalance changes — but only after initial load completed
  useEffect(() => {
    // don't attempt refresh if we haven't finished first load for this user
    if (!initialLoadedRef.current) return;

    // perform quiet refresh (background)
    loadVaults({ quiet: true });

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [solBalance]);

  // Helper to resolve USD price for a token symbol
  // Prefer live price in `prices` (Pyth), fallback to bySymbol.usdPrice
  const resolveUsdPrice = (symbol?: string | null): number | null => {
    if (!symbol) return null;
    const key = symbol.toString().toUpperCase();

    // Pyth price object shape from your hook: { price: number, conf, expo, ... }
    const pyth = (prices as any)?.[key];
    if (pyth && typeof pyth.price === "number") {
      return pyth.price;
    }

    const agg = (bySymbol as any)?.[key];
    if (agg && typeof agg.usdPrice === "number") {
      return agg.usdPrice;
    }
  ...
  // compute total USD value of vaults (recomputes when vault list or prices change)
  const totalUsd = useMemo(() => {
    if (!vaults || vaults.length === 0) return 0;
    let total = 0;
    for (const v of vaults) {
      const token = (v.token ?? "SOL").toString().toUpperCase();
      const amt = typeof v.amount === "string" ? Number(v.amount) || 0 : (v.amount ?? 0);
      const price = resolveUsdPrice(token);
      if (price != null && !Number.isNaN(price)) {
        total += amt * price;
      } else {
        // if price is missing, skip (treat as 0). Optionally you can still attempt to approximate.
      }
    }
    return total;
    // include `prices` and `bySymbol` to react to price updates from useWalletBalancesRealtime
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [vaults, JSON.stringify(prices ?? {}), JSON.stringify(bySymbol ?? {})]);
  ...
  return (
    <div className="bg-[#27AAE1] rounded-2xl p-4 md:p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-white text-sm">Vault Balance</h2>
      </div>
      <div className="mb-4">
        <h1 className="text-3xl md:text-4xl font-bold text-white ">{loading ? "-" : fmtCurrency(totalUsd)}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
        ...
        <div className="relative inline-block group w-full">
          <button
            ...
            aria-disabled="true"
            tabIndex={-1}
            className="bg-[#658BEC] w-full text-white py-2 md:py-3 px-2 md:px-4 rounded-[20.5px] text-[11px] md:text-[13px] font-inter font-medium flex items-center justify-center space-x-1 md:space-x-2 transition-all duration-200 opacity-60 cursor-not-allowed"
          >
          ...
          <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 bottom-full mb-3 w-max rounded-md bg-black/90 px-3 py-1 text-xs text-white opacity-0 scale-95 translate-y-1 transition-all duration-150 group-hover:opacity-100 group-focus-within:opacity-100 group-hover:translate-y-0 group-focus-within:translate-y-0">
            Coming soon
          </div>
        </div>
